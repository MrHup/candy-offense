testPhysics = gideros.class(Sprite)
require "box2d"
...
function testPhysics:character(i,j,width,height)
...
function testPhysics:wall(x, y, width, height)
...
function testPhysics:solid(x, y, img)
...
function testPhysics:enemy(i,j,pack,speed,damage,spec) -- pozitie, set de imagini, viteza, damage, special power
	local enemy = Bitmap.new(Texture.new("Images/debug_enemy.png"))
	enemy.health = 100
	
	enemy:setAnchorPoint(0.5,0.5)
	setSize(enemy,conf.dimens,conf.dimens)
	enemy:setPosition(j*conf.dimens+conf.dimens/2,i*conf.dimens+conf.dimens/2)
	
	enemies:addChild(enemy)
	
	local vox = {} -- in the honour of vox populis | auxiliar array
	copy_array(grid,vox)
	local lista1 = {}
	lista1 = CalcPath(CalcMoves(vox, i, j, 11, 3))
	
	local lista2= {}
	for i=1, table.getn(lista1) do
		lista2[i]={}
		lista2[i].x=lista1[i].x
		lista2[i].y=lista1[i].y
	end
	
	local function autodestroy()
		if enemy ~= nil and enemies:contains(enemy) then
			HP = HP - damage
			print("Ai fost lovit: " .. HP)
	...
	local rawSpeed = 10
	local segment = 4
	local function move_inSegments(x,y,fx,fy,seg) -- mai putine seg = animatie mai fluida, fx si fy is poz. finale
		...
		enemy:setPosition(aux_x,aux_y)
		if not (aux_x == fx and aux_y== fy) then
			Timer.delayedCall(rawSpeed, function() move_inSegments(aux_x,aux_y,fx,fy,seg) end)
		end
	end
	
	local function move_pawn(i,j,k)
		--enemy:setPosition(j*conf.dimens+conf.dimens/2,i*conf.dimens+conf.dimens/2)
		move_inSegments(enemy:getX(),enemy:getY(),j*conf.dimens+conf.dimens/2,i*conf.dimens+conf.dimens/2,segment)
		if k < table.getn(lista1) then
			Timer.delayedCall(rawSpeed*10+200, function() move_pawn(lista2[k].x,lista2[k].y,k+1) end)
		else
			Timer.delayedCall(800, function() autodestroy() end)
		end
	end
	Timer.delayedCall(500, function() move_pawn(lista2[1].x,lista2[1].y,1) end)
	helpers.enemy_count = helpers.enemy_count + 1
	return enemy
end
------
function testPhysics:portal(i,j,spawnTime)
	local portal = Bitmap.new(Texture.new("Images/debug_portal.png"))
	local stop = false
	...
	setSize(portal,conf.dimens,conf.dimens)
	portal:setPosition(j*conf.dimens+conf.dimens/2,i*conf.dimens+conf.dimens/2)
	
	portal_group:addChild(portal)
	
	--[[local function autodestroy()
		stop = true
		self:removeChild(portal)
	end
	Timer.delayedCall(25000, function() autodestroy() end)]]
	...
	--[[local function spawn_enemy()
		...
		if stop == false then
			Timer.delayedCall(spawnTime, function() spawn_enemy() end)
		end
	end
	
	Timer.delayedCall(spawnTime + 500, function() spawn_enemy() end)]]
...
function testPhysics:deseneaza()
...
function testPhysics:check_for_empty()
...
local Si=0
local Sj=1
function testPhysics:rutina(pool,delay,m,n)
...
function testPhysics:start_game()
...
---------------------ENTER FRAME----------------------------
------------------------------------------------------------
function testPhysics:onEnterFrame()
